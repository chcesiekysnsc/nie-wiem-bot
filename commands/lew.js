...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!isOwner) {
      const cooldownState = await checkCooldown('lew', senderId);
    ...
    await message.reply('🦁 Szukam zdjęcia lwa...').catch(() => null);

    try {
      const imageUrl = await fetchRedditImage('lions');
      ...
      if (client.api && threadId) {
        ...
        const tempFile = path.join(__dirname, `temp_lew_${Date.now()}.${ext}`);
