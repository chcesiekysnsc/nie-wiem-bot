...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (playerGame.state !== 'night') {
      await message.reply('❌ Ruchy lekarza można wysyłać tylko w nocy.');
      return;
    }
    ...
    if (playerObj.role !== 'Lekarz') {
      await message.reply('❌ Nie jesteś Lekarzem! Ta komenda jest zarezerwowana dla Lekarza.');
      return;
    }

    const targetArg = args[0];
    if (!targetArg) {
      await message.reply('❌ Użyj: **!ulecz <numer_gracza_z_listy>**');
      return;
    }
    ...
    if (!targetPlayer.alive) {
      await message.reply('❌ Ten gracz już nie żyje, nie możesz go uleczyć.');
      return;
    }

    playerGame.doctorTarget = targetPlayer.id;
    await message.reply(`✅ **Zlecenie przyjęte!** Tej nocy będziesz chronić gracza **${targetPlayer.name}**.`);
  }
};
