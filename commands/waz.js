...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!isOwner) {
      const cooldownState = await checkCooldown('waz', senderId);
    ...
    await message.reply('🐍 Szukam zdjęcia węża...').catch(() => null);

    try {
      const imageUrl = await fetchRedditImage('snakes');
      ...
      if (client.api && threadId) {
        ...
        const tempFile = path.join(__dirname, `temp_waz_${Date.now()}.${ext}`);
