...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!isOwner) {
      const cooldownState = await checkCooldown('kameleon', senderId);
    ...
    await message.reply('🦎 Szukam zdjęcia kameleona...').catch(() => null);

    try {
      const imageUrl = await fetchRedditImage('chameleons');
      ...
      if (client.api && threadId) {
        ...
        const tempFile = path.join(__dirname, `temp_kameleon_${Date.now()}.${ext}`);
      ...
      console.error('[KAMELEON] Error:', err);
      await message.reply('❌ Nie udało się pobrać obrazka, spróbuj ponownie później.').catch(() => null);
    }
  }
};
