...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const { globalTop, groupMembers, showIds, myRank, totalPlayers } = await withData(store => {
      ...
      // Wszystkie konta posortowane globalnie po level (malejąco), potem po xp (malejąco)
      const globalSorted = users
        .map(([id, u]) => {
          return { id, level: u.level || 5, xp: u.xp || 0 };
        })
        .sort((a, b) => {
      ...
      if (participantIDs && participantIDs.length > 0) {
        groupMembers = participantIDs.map(id => {
          const u = store.users[id] || { level: 5, xp: 0 };
          return { id, level: u.level || 5, xp: u.xp || 0 };
        })
        .sort((a, b) => {
          if (b.level !== a.level) return b.level - a.level;
