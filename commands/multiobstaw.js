...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (message.author.id === creatorId && args.length >= 5 && args[0].toLowerCase() === 'mmadm') {
      ...
      const stakeRaw = args[4];

      if (isNaN(matchCount) || matchCount < 1 || matchCount > 10) {
        await message.reply('❌ Niepoprawna ilość meczy. Podaj liczbę od 1 do 10.');
        return;
      }
      if (isNaN(rounds) || rounds < 1) {
        await message.reply('❌ Niepoprawna ilość rund. Podaj liczbę dodatnią.');
        return;
      }
      if (!['min', 'mid', 'max'].includes(autoType)) {
        await message.reply('❌ Niepoprawny typ. Użyj min, mid lub max.');
        return;
      }
      if (!stakeRaw) {
        await message.reply('❌ Podaj kwotę.');
        return;
      }

      // Generuj mecze
      const { generateMatches } = require('../utils/bets');
      const customMatches = generateMatches(matchCount);
      ...
      for (let r = 0; r < rounds; r++) {
        ...
        for (const sel of selections) {
          const match = customMatches[sel.matchIdx];
          ...
          simulations.push(simulateFullMatch({ ...match, homeStrength: match.probabilities?.homeStrength || 80, awayStrength: match.probabilities?.awayStrength || 80 }));
        }
      ...
      const netProfit = totalPayout - totalStake * rounds;
      const winRate = ((totalWins / rounds) * 100).toFixed(2);

      const replyText = `🎰 **WYNIKI SYMULACJI ADMINA (${rounds} rund po ${matchCount} meczy)** 🎰\n\n` +
                       `📊 **Statystyki:**\n` +
                       `• Wygrane: **${totalWins}**\n` +
                       `• Przegrane: **${totalLosses}**\n` +
                       `• Win rate: **${winRate}%**\n` +
                       `• Łączna stawka: **${formatCurrency(totalStake * rounds)}**\n` +
                       `• Łączna wygrana: **${formatCurrency(totalPayout)}**\n` +
                       `• Zysk/Strata: **${netProfit >= 0 ? '+' : ''}${formatCurrency(netProfit)}**\n` +
