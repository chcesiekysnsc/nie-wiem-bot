...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!isOwner) {
      const cooldownState = await checkCooldown('jaszczurka', senderId);
    ...
    await message.reply('🦎 Szukam zdjęcia jaszczurki...').catch(() => null);

    try {
      const imageUrl = await fetchRedditImage('lizards');
      ...
      if (client.api && threadId) {
        ...
        const tempFile = path.join(__dirname, `temp_jaszczurka_${Date.now()}.${ext}`);
      ...
      console.error('[JASZCZURKA] Error:', err);
      await message.reply('❌ Nie udało się pobrać obrazka, spróbuj ponownie później.').catch(() => null);
    }
  }
};
