...
function getSectionHTML(html, id) {
  ...
  const nextSectionRegex = /id="(top-artists|top-albums|top-tracks|recent-tracks-section|mpu_|leader_)/gi;
  
  // We need to find the first match of nextSectionRegex
  let match;
  let nextIndex = -1;
  while ((match = nextSectionRegex.exec(content)) !== null) {
...
function parseSection(id) {
  const sectionContent = getSectionHTML(html, id);
  const items = [];
