...
const CHALLENGES = [
  { id: 'work_35', type: 'work_count', label: 'Pracownik miesiaca', description: 'Uzyj !work 35 razy', target: 35, reward: 180000 },
  { id: 'work_21', type: 'work_count', label: 'Pracownik tygodnia', description: 'Uzyj !work 21 razy', target: 21, reward: 75000 },
  { id: 'rob_4', type: 'rob_count', label: 'Zlodziej', description: 'Okradnij kogos 4 razy (!rob)', target: 4, reward: 130000 },
  { id: 'cf_streak_4', type: 'coinflip_streak', label: 'Farciarz', description: 'Wygrywaj !coinflip 4 razy pod rzad (min. 10k stawka)', target: 4, minBet: 10000, reward: 120000 },
  { id: 'crime_10', type: 'crime_count', label: 'Przestepca', description: 'Uzyj !crime 10 razy', target: 10, reward: 200000 },
  { id: 'pkn_streak_3', type: 'pkn_streak', label: 'Manipulant', description: 'Wygrywaj !pkn 3 razy pod rzad (min. 20k stawka)', target: 3, minBet: 20000, reward: 200000 },
  { id: 'slots_streak_5', type: 'slots_streak', label: 'Dzordzo 5', description: 'Wygrywaj !slots 5 razy pod rzad (min. 20k stawka)', target: 5, minBet: 20000, reward: 220000 },
  { id: 'ruletka_streak_5', type: 'ruletka_streak', label: 'Urodzony hazardzista', description: 'Wygrywaj !ruletka 5 razy pod rzad (min. 20k stawka)', target: 5, minBet: 20000, reward: 300000 },
  ...
  { id: 'cr_hardcore_4', type: 'chickenroad_hardcore_4', label: 'Wilson lo siento', description: 'Dojdz do 4. pasa i wyplac w !chickenroad Hardcore (min. 30k stawka)', target: 4, minBet: 30000, difficulty: 'hardcore', reward: 360000 },
  { id: 'wojna_streak_3', type: 'wojna_streak', label: 'Wojenny weteran', description: 'Wygraj w !wojna 3 razy pod rzad (min. 30k stawka)', target: 3, minBet: 30000, reward: 130000 },
  { id: 'bj_3', type: 'blackjack_wins', label: 'Mistrz Blackjacka', description: 'Wygrywaj w !blackjack 3 razy (kazda wygrana reka liczy sie, min. 30k stawka)', target: 3, minBet: 30000, reward: 150000 },
  { id: 'cr_1', type: 'chickenroad_wins', label: 'Kurczak na trasie', description: 'Ukoncz 1 gre w !chickenroad docierajac do ostatniego pasa (min. poziom Hard, min. 25k stawka)', target: 1, minBet: 25000, difficulty: 'hard', reward: 425000 }
];
...
function advanceChallenge(userId, store, type, amount = 1, meta = {}) {
  ...
  } else if (type === 'chickenroad_wins') {
    const difficulty = meta.difficulty || '';
    const betAmount = meta.betAmount || 0;
    const minBet = challenge.minBet || 0;
    const requiredDifficulty = (challenge.difficulty || 'hard').toLowerCase();

    if (requiredDifficulty && difficulty !== requiredDifficulty) return challenge;
    if (betAmount < minBet) return challenge;
    if (!meta.reachedEnd) return challenge;
